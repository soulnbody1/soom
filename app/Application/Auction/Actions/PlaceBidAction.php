    ...
    public function execute(
        ...
        return $this->transaction->run(function () use ($auction, $bidderId, $amount, $currency, $idempotencyKey, $clientRequestId): AuctionBid {
            ...
            if ($auction->seller_id === $bidderId) {
                throw AuctionException::bidRejected('Seller cannot bid on own auction.');
            }

            if ($auction->status !== AuctionStatus::Live || ! $auction->starts_at || ! $auction->ends_at) {
                throw AuctionException::bidRejected('Auction is not live.');
            }

            if ($auction->starts_at->greaterThan($now) || ! $now->lessThan($auction->ends_at)) {
                throw AuctionException::bidRejected('Auction is outside its bidding window.');
            }
            ...
            if (! $participant || $participant->status !== AuctionParticipantStatus::Qualified) {
                throw AuctionException::bidRejected('Bidder is not qualified for this auction.');
            }
            ...
            if (! $acceptedTerms) {
                throw AuctionException::bidRejected('Auction terms must be accepted before bidding.');
            }
            ...
            if (! $deposit || $deposit->status !== AuctionDepositStatus::Held || $deposit->held_amount_minor < $auction->bidder_deposit_amount_minor) {
                throw AuctionException::bidRejected('Approved bidder deposit is required before bidding.');
            }

            $bidMoney = Money::fromDecimalString($amount, strtoupper($currency));
            if ($bidMoney->currency !== $auction->currency_code) {
                throw AuctionException::bidRejected('Bid currency does not match auction currency.');
            }
            ...
            if ($bidMoney->minor < $minimum) {
                throw AuctionException::bidRejected('Bid amount is below the required minimum.');
            }
